...
window.onresize = () => {
	...
	adjustBoardSize(screenHeight, screenWidth);
};
...
function gamePlay(e) {
	//Handle the first click only
	if (e.detail === 1) {
		let thisPlayer = player.textContent;
		if (playOver.textContent === '') {
			...
			if ((row = getRowCol(col))) {
				//Get available td / row col if row is truthy
...
function adjustBoardSize(screenHeight, screenWidth) {
	screenHeight > screenWidth ? (size = screenWidth) : (size = screenHeight);
	size = size * 0.8;
	document.querySelector('.game').setAttribute('height', size);
	document.querySelector('.game').setAttribute('width', size);
	document.querySelector('.colHead').setAttribute('width', size / WIDTH);
	document.querySelector('.colHead').setAttribute('height', size / WIDTH);
	const cols = document.querySelectorAll('.col');
	for (let col of cols) {
		col.setAttribute('width', size / WIDTH);
		col.setAttribute('height', size / WIDTH);
	}
}

function getPlayer() {
	//Get the current player's color to display
...
function getRowCol(col) {
	//Get the next vacant row number based on the column selected
